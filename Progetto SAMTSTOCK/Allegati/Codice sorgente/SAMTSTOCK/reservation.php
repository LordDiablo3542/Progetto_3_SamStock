	...
	<body>
		<div class="container">
			...
			<?php
				...
				$result = mysqli_query($con,"SELECT * FROM prodotti, categorie, utenti WHERE prodotti.Categoria = categorie.ID_Categoria AND prodotti.Riservato = utenti.ID_Utente AND Responsabile = '$idutente' AND Riservato IS NOT NULL"); //seleziono i prodotti di mia responsabilità riservati
					...
					while($row = mysqli_fetch_array($result)) {
						...
						if($row['Quantita'] > 1){
							...
							echo '<td class="sizedTDL">
									...
									<a href="reservation.php?back=true&nomep='. $row['NomeP'] .'&emailu='. $row['EMail'] .'" class="actionLinks" title="Avviso di riconsegna"> 
										<i class="glyphicon glyphicon-time"></i>
									</a>
									<a class="actionLinks" title="Anulla riservazione" onclick="deleterFunction('. $row['ID_Prodotto'] .')"> 
		...
		<?php
			...
			//se back è settato setto l'e-mail in riporta indietro
			if (isset($_GET['back'])) {
				$nomep = $_GET['nomep'];
				$to = $_GET['emailu'];
				$subject = "Riportare:  ". $nomep;
				$message = "Il prestito sta per scadere.\r\nSei pregato di riportare il prodotto (" . $nomep .") entro la fine di questa settimana.\r\n\r\n" . $name;
			...
			function emailSender($to, $subject, $message, $email) {
				...
				if($retval == true) {
					header("location: reservation.php?send=true"); //redirect giusto
				}else {
					header("location: reservation.php?nosend=true"); //redirect errore
				}
			}
		?>
		...
		<script>
			...
			function deleterFunction(id) {
				if (confirm("Sei sicuro di voler cancellare questa riservazione?") == true) { //chiede se sei sicuro
