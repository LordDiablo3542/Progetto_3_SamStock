					...
					<div class="col-xs-3 col-sm-3 col-md-3">
    					<div class="form-group">
							...
							<div class="input-group">
								...
								<input type="number" class="form-control input-lg" placeholder="Prezzo" name="prezzo" min="0">
		...
		<?php			
			...
			if(!empty($nomeProdotto) && !empty($categoria) && !empty($nDiSerie)){
				...
				if($quantita > 1){
					...
					if (!mysqli_query($con,"INSERT INTO prodotti (ID_Prodotto, NomeP, Categoria, Modello, `Numero di serie`, Disponibile, Portabile, Icona, Aula, DateTime, Descrizione, Riservato, Responsabile, Prezzo, Quantita)
					SELECT NULL, '$nomeProdotto', ID_Categoria, '$modello', '$nDiSerie', '$disponibile', '$portabile', '$icona', '$aula', NOW(), '$descrizione', '$idutente', '$idutente', '$prezzo', '$quantita' FROM categorie WHERE NomeC = '$categoria'")) {
						die(header("location: items.php?nameerr=true")); //errore
					}
				}else{
					//query con select dall'altra tabella dell'ID del nome della categoria
					if (!mysqli_query($con,"INSERT INTO prodotti (ID_Prodotto, NomeP, Categoria, Modello, `Numero di serie`, Disponibile, Portabile, Icona, Aula, DateTime, Descrizione, Responsabile, Prezzo, Quantita)
					SELECT NULL, '$nomeProdotto', ID_Categoria, '$modello', '$nDiSerie', '$disponibile', '$portabile', '$icona', '$aula', NOW(), '$descrizione', '$idutente', '$prezzo', '$quantita' FROM categorie WHERE NomeC = '$categoria'")) {
						die(header("location: items.php?nameerr=true")); //errore
					}
				}
